...
def main() -> None:
    ...
    total_nodes = len(parsed_migration.ast_nodes) * MIGRATION_COUNT
    print(f"Total AST Nodes to Process: {total_nodes}")
...
- **Test Payload:** 1,000 duplicated migrations containing `CREATE TABLE`,
  `CREATE INDEX`, `ALTER TABLE`, and `DROP TABLE` statements.
...
- **Errors / Warnings / Infos:** {result.total_errors} / {result.total_warnings} \\
  / {result.total_infos}

The engine demonstrates efficient throughput traversing tens of thousands of AST nodes
synchronously.
"""
